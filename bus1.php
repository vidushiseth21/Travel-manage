    ...
    <center>
          ...
          <?php
              ...
              if(!empty($_POST['rdate']))
                $sql = "select * from bus_trips where (deptfrom like '$dfrom%' and goingto like '$gto%' and tourdate = '$ddate') or    (deptfrom like '$gto%' and goingto like '$dfrom%' and tourdate = '$rdate') order by bus_id desc";

              else
               $sql = "select * from bus_trips where deptfrom like '$dfrom%' and goingto like '$gto%' and tourdate = '$ddate' order by bus_id desc";          
            ...
            <?php
              while($row = mysqli_fetch_array($result)) {
               ...
               $_SESSION["route"] =  $row['deptfrom'] ." - ". $row['goingto']?>              
                 ...
                 <td onclick="document.location = 'busSeat.php?id=<?php echo urlencode($bid);?>'" style="background-color: 728FCE; color: white; font-size: 20px; cursor:hand;">Show Seat</td>
                </tr>                
          <?php
          }
        }
